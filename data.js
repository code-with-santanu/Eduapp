let data = {
	roadmap: [
		{
			course: "Backend Development",
			topics: [
				{
					name: "Introduction to Backend Development",
					subtopics: [
						{
							name: "What is Backend Development",
							link: "https://www.youtube.com/watch?v=cbSrsYiRamo",
						},
						{
							name: "Introduction to Backend Development",
							link: "https://dev.to/kaperskyguru/introduction-to-backend-development-506a",
						},
						{
							name: "Back-end Developer Career Path",
							link: "https://www.boot.dev/tracks/backend",
						},
						{
							name: "How Backend works",
							link: "https://www.youtube.com/watch?v=4r6WdaY3SOA",
						},
						{
							name: "Guide to Backend Development",
							link: "https://www.upwork.com/resources/beginners-guide-back-end-development",
						},
					],
				},
				{
					name: "Basics of Frontend",
					subtopics: [
						{
							name: "HTML",
							link: "https://www.youtube.com/watch?v=kUMe1FH4CHE",
						},
						{
							name: "CSS",
							link: "https://www.youtube.com/watch?v=1Rs2ND1ryYc",
						},
						{
							name: "Frontend Developer Course",
							link: "https://www.youtube.com/watch?v=QA0XpGhiz5w",
						},
					],
				},
				{
					name: "Javascript",
					subtopics: [
						{
							name: "Nodejs",
							link: "https://www.youtube.com/watch?v=fBNz5xF-Kx4",
						},
						{
							name: "Expressjs",
							link: "https://expressjs.com/en/starter/installing.html",
						},
					],
				},
				{
					name: "Version Control",
					subtopics: [
						{
							name: "Git documentation",
							link: "https://git-scm.com/docs/gittutorial",
						},
						{
							name: "Git tutorial",
							link: "https://www.youtube.com/watch?v=apGV9Kg7ics&t=1964s",
						},
					],
				},
				{
					name: "Relational Databases",
					subtopics: [
						{
							name: "Learn Sql and Relational Databases",
							link: "https://www.boot.dev/learn/learn-sql",
						},
						{
							name: "PostgreSql",
							link: "https://www.youtube.com/watch?v=qw--VYLpxG4",
						},
					],
				},
				{
					name: "NoSQL Databases",
					subtopics: [
						{
							name: "What is nosql database",
							link: "https://www.youtube.com/watch?v=uD3p_rZPBUQ",
						},
						{
							name: "MongoDB",
							link: "https://www.youtube.com/watch?v=Www6cTUymCY",
						},
					],
				},
				{
					name: "APIs",
					subtopics: [
						{
							name: "Learn to build APIs",
							link: "https://www.youtube.com/watch?v=pKd0Rpw7O48",
						},
						{
							name: "Client side caching",
							link: "https://www.youtube.com/watch?v=HiBDZgTNpXY",
						},
						{
							name: "Serversidecaching",
							link: "https://www.starwindsoftware.com/resource-library/server-side-caching/",
						},
						{
							name: "CDN",
							link: "https://www.cloudflare.com/en-ca/learning/cdn/what-is-a-cdn/",
						},
					],
				},
				{
					name: "Web Servers",
					subtopics: [
						{
							name: "What are web servers?",
							link: "https://www.youtube.com/watch?v=JhpUch6lWMw",
						},
						{
							name: "What is a web server? - NGINX",
							link: "https://www.nginx.com/resources/glossary/web-server/",
						},
					],
				},
				{
					name: "Build Projects",
					subtopics: [
						{
							name: "4 projects with Node.js/Express",
							link: "https://www.youtube.com/watch?v=qwfE7fSVaZM",
						},
					],
				},
			],
		},
		{
			course: "Android Development",
			topics: [
				{
					name: "Introduction",
					subtopics: [
						{
							name: "Introduction",
							link: "https://google-developer-training.github.io/android-developer-fundamentals-course-concepts-v2/unit-1-get-started/lesson-1-build-your-first-app/1-0-c-introduction-to-android/1-0-c-introduction-to-android.html",
						},
						{
							name: "Android Studio Setup",
							link: "https://www.youtube.com/watch?v=4M0hNugPJV8",
						},
					],
				},
				{
					name: "Introduction To Kotlin",
					subtopics: [
						{
							name: "Kotlin Course",
							link: "https://developer.android.com/courses/pathways/android-basics-kotlin-one",
						},
						{
							name: "Notes-Slides",
							link: "https://piazza.com/class_profile/get_resource/ktlu0ly5db84bb/kubl00rkflpx8",
						},
					],
				},
				{
					name: "Layouts And UI",
					subtopics: [
						{
							name: "Layout and UI",
							link: "https://google-developer-training.github.io/android-developer-fundamentals-course-concepts-v2/unit-1-get-started/lesson-1-build-your-first-app/1-2-c-layouts-and-resources-for-the-ui/1-2-c-layouts-and-resources-for-the-ui.html",
						},
					],
				},
				{
					name: "Event Based Programming",
					subtopics: [
						{
							name: "Developer Docs",
							link: "https://developer.android.com/develop/ui/views/touch-and-input/input-events",
						},
						{
							name: "Event Handlers",
							link: "https://www.youtube.com/watch?v=LV_5lOvYAn8",
						},
					],
				},
				{
					name: "Networking And API's",
					subtopics: [
						{
							name: "Networking And API's",
							link: "https://www.youtube.com/watch?v=k2N3EoZI3eU",
						},
						{ name: "Retrofit", link: "https://square.github.io/retrofit/" },
					],
				},
				{
					name: "Application Architecture",
					subtopics: [
						{
							name: "Article on App Architecture",
							link: "https://developer.android.com/topic/architecture",
						},
						{
							name: "Android's Guide To App Architecture",
							link: "https://www.youtube.com/watch?v=4L3-DW-z7iI",
						},
					],
				},
				{
					name: "Projects",
					subtopics: [
						{
							name: "Tip Calculator Application",
							link: "https://www.youtube.com/playlist?list=PL7NYbSE8uaBCMVBVg6cskGzdYguj3CUP-",
						},
						{
							name: "My Maps Application",
							link: "https://www.youtube.com/playlist?list=PL7NYbSE8uaBCSkZum6Z88RvjiXrTBpjT2",
						},
					],
				},
			],
		},
		{
			course: "Blockchain",
			topics: [
				{
					name: "Introduction",
					subtopics: [
						{
							name: "What is Web3",
							link: "https://www.freecodecamp.org/news/what-is-web3/",
						},
					],
				},
				{
					name: "Blockchain Fundamentals",
					subtopics: [
						{
							name: "What is Blockchain",
							link: "https://www.geeksforgeeks.org/blockchain-technology-introduction/",
						},
						{
							name: "Distributed Ledger",
							link: "https://www.geeksforgeeks.org/blockchain-and-distributed-ledger-technology-dlt/",
						},
						{
							name: "Crypto Basics",
							link: "https://www.oswego.edu/cts/basics-about-cryptocurrency",
						},
						{
							name: "Difference between Web1,Web2,Web3",
							link: "https://www.simplilearn.com/what-is-web-1-0-web-2-0-and-web-3-0-with-their-difference-article",
						},
						{
							name: "NFT",
							link: "https://www.forbes.com/advisor/investing/cryptocurrency/nft-non-fungible-token/",
						},
						{
							name: "Smart Contracts",
							link: "https://ethereum.org/en/developers/docs/smart-contracts/",
						},
					],
				},
				{
					name: "Choose A Blockchain",
					subtopics: [
						{ name: "Ethereum", link: "https://ethereum.org/en/" },
						{ name: "Solana", link: "https://solana.com/" },
					],
				},
				{
					name: "Ethereum Concepts",
					subtopics: [
						{
							name: "Accounts",
							link: "https://ethereum.org/en/developers/docs/accounts/",
						},
						{
							name: "Transactions",
							link: "https://ethereum.org/en/developers/docs/transactions/",
						},
						{
							name: "Ethereum Virtual Machine",
							link: "https://ethereum.org/en/developers/docs/evm/",
						},
						{
							name: "Gas",
							link: "https://ethereum.org/en/developers/docs/gas/",
						},
						{
							name: "Nodes And Clients",
							link: "https://ethereum.org/en/developers/docs/nodes-and-clients/",
						},
					],
				},
				{
					name: "Layer 2 Solutions",
					subtopics: [
						{ name: "Polygon", link: "https://wiki.polygon.technology/" },
					],
				},
				{
					name: "Solidity",
					subtopics: [
						{
							name: "Solidity With Javascript 31 hours Course",
							link: "https://www.youtube.com/watch?v=gyMwXuJrbJQ&feature=youtu.be",
						},
						{
							name: "Solidity Docs",
							link: " https://docs.soliditylang.org/en/v0.8.16/",
						},
						{ name: "CryptoZombies", link: "https://cryptozombies.io/" },
					],
				},
				{
					name: "Connect Dapp to Frontend",
					subtopics: [
						{
							name: "Web3.js",
							link: "https://web3js.readthedocs.io/en/v1.7.5/",
						},
						{ name: "Ether.js", link: "https://docs.ethers.org/v5/" },
					],
				},
				{
					name: "Mid-Level Tools",
					subtopics: [
						{ name: "Open Zeppelin", link: "https://docs.openzeppelin.com/" },
						{
							name: "Wallet Connect",
							link: "https://docs.walletconnect.com/2.0",
						},
						{ name: "Chain Link", link: "https://docs.chain.link/" },
						{
							name: "Ceramic Network",
							link: "https://developers.ceramic.network/learn/welcome/",
						},
					],
				},
				{
					name: "SDKs",
					subtopics: [
						{ name: "Third Web", link: "https://portal.thirdweb.com/" },
						{
							name: "Moralis",
							link: "https://docs.moralis.io/web3-data-api/evm/getting-started",
						},
					],
				},
				{
					name: "Projects",
					subtopics: [
						{
							name: "Moralis Web3 (Youtube)",
							link: "https://www.youtube.com/c/MoralisWeb3",
						},
						{
							name: "Alchemy (Youtube)",
							link: "https://www.youtube.com/c/AlchemyPlatform",
						},
						{
							name: "The Complete Guide to Full Stack Web3 Development",
							link: "https://dev.to/edge-and-node/the-complete-guide-to-full-stack-web3-development-4g74",
						},
					],
				},
			],
		},
		{
			course: "Data-Science",
			topics: [
				{
					name: "What Is Data Science",
					subtopics: [
						{
							name: "what is data science?",
							link: "https://www.ibm.com/topics/data-science",
						},
					],
				},
				{
					name: "Programming Languages",
					subtopics: [
						{
							name: "R",
							link: "https://www.youtube.com/watch?v=_V8eKsto3Ug&feature=youtu.be",
						},
						{
							name: "Python",
							link: "https://www.youtube.com/watch?v=t8pPdKYpowI&feature=youtu.be",
						},
						{
							name: "SQL",
							link: "https://www.youtube.com/watch?v=HXV3zeQKqGY",
						},
					],
				},
				{
					name: "Frameworks",
					subtopics: [
						{
							name: "Pandas",
							link: "https://www.youtube.com/playlist?list=PL-osiE80TeTsWmV9i9c58mdDCSskIFdDS",
						},
						{
							name: "Numpy",
							link: "https://www.youtube.com/watch?v=QUT1VHiLmmI",
						},
						{
							name: "Scipy",
							link: "https://www.youtube.com/watch?v=jmX4FOUEfgU",
						},
					],
				},
				{
					name: "Mathematics for data science",
					subtopics: [
						{
							name: "Mathematics for data science",
							link: "https://www.youtube.com/playlist?list=PLeo1K3hjS3uuKaU2nBDwr6zrSOTzNCs0l",
						},
						{
							name: "Maths for data science -GFG(GeeksForGeeks)",
							link: "https://www.geeksforgeeks.org/maths-for-data-science/",
						},
					],
				},
				{
					name: "Probability:",
					subtopics: [
						{
							name: "Probability and stastics for data science -Khan Academy",
							link: "https://www.khanacademy.org/math/statistics-probability/probability-library",
						},
					],
				},
				{
					name: "Statistics",
					subtopics: [
						{
							name: "Statistics fundamentals -Youtube(StatQuest with Josh Starmer)",
							link: "https://www.youtube.com/playlist?list=PLblh5JKOoLUK0FLuzwntyYI10UQFUhsY9",
						},
						{
							name: "Statistics for data science -Youtube(Great Learning)",
							link: "https://www.youtube.com/watch?v=Vfo5le26IhY",
						},
					],
				},
				{
					name: "Data Manipulation",
					subtopics: [
						{
							name: "Data cleaning",
							link: "https://www.youtube.com/watch?v=bDhvCp3_lYw",
						},
						{
							name: "Data Visualization",
							link: "https://www.youtube.com/watch?v=UO98lJQ3QGI&list=PL-osiE80TeTvipOqomVEeZ1HRrcEvtZB_",
						},
						{
							name: "EDA",
							link: "https://www.youtube.com/watch?v=fHFOANOHwh8",
						},
					],
				},
				{
					name: "Machine Learning",
					subtopics: [
						{
							name: "Machine learning playlist",
							link: "https://www.youtube.com/playlist?list=PLoROMvodv4rMiGQp3WXShtMGgzqpfVfbU",
						},
						{
							name: "ML Documentation",
							link: "https://www.geeksforgeeks.org/machine-learning/",
						},
					],
				},
				{
					name: "Model Deployment",
					subtopics: [
						{
							name: "Flask tutorial -GFG(geeksforgeeks)",
							link: "https://www.geeksforgeeks.org/flask-creating-first-simple-application/",
						},
						{
							name: "Deploy Models with TensorFlow Serving and Flask -Coursera(Amit Yadav)",
							link: "https://www.coursera.org/projects/deploy-models-tensorflow-serving-flask",
						},
					],
				},
				{
					name: "Project Ideas",
					subtopics: [
						{
							name: "Machine Learning Project",
							link: "https://www.youtube.com/watch?v=fiz1ORTBGpY&list=PLfFghEzKVmjvuSA67LszN1dZ-Dd_pkus6",
						},
						{
							name: "End To End ML Project With Dockers,Github Actions And Deployment",
							link: "https://www.youtube.com/watch?v=MJ1vWb1rGwM",
						},
					],
				},
			],
		},
		{
			course: "DevOps",
			topics: [
				{
					name: "GoLang",
					subtopics: [
						{
							name: "Go tutorial - Tutorials point",
							link: "https://www.tutorialspoint.com/go/index.htm",
						},
						{
							name: "Complete Golang tutorial - Hitesh Choudhary(Youtube)",
							link: "https://www.youtube.com/playlist?list=PLRAV69dS1uWQGDQoBYMZWKjzuhCaOnBpa",
						},
					],
				},
				// {
				//   name: "GoLang",
				//   subtopics: [
				//     {
				//       name: "Go tutorial - Tutorials point",
				//       link: "https://www.tutorialspoint.com/go/index.htm",
				//     },
				//     {
				//       name: "Complete Golang tutorial - Hitesh Choudhary(Youtube)",
				//       link: "https://www.youtube.com/playlist?list=PLRAV69dS1uWQGDQoBYMZWKjzuhCaOnBpa",
				//     },
				//   ],
				// },
				{
					name: "Linux, Terminal Basics",
					subtopics: [
						{
							name: "Bogdan Stashchuk - Linux for beginners playlist",
							link: "https://www.youtube.com/playlist?list=PLWkguCWKqN9OrwsklvLC8FB87TbaOQGqY",
						},
						{ "Linux Complete Hands-On Tutorial": "https://linuxjourney.com/" },
					],
				},
				{
					name: "Operating, System Basics",
					subtopics: [
						{
							name: "Introduction to operating systems - Jenny's Lectures(Youtube)",
							link: "https://www.youtube.com/watch?v=RozoeWzT7IM",
						},
						{
							name: "Operating system tutorial - Tutorialspoint",
							link: "https://www.tutorialspoint.com/operating_system/index.htm#:~:text=An%20Operating%20System%20(OS)%20is%20an%20interface%20between%20a%20computer,as%20disk%20drives%20and%20printers.",
						},
					],
				},
				{
					name: "Server, Management",
					subtopics: [
						{
							name: "Linux For Server Management",
							link: "https://www.youtube.com/watch?v=HsDIz0zKwjs",
						},
						{
							name: "Linux System Administration Basics",
							link: "https://www.linode.com/docs/guides/linux-system-administration-basics/",
						},
					],
				},
				{
					name: "Shell, Scripting",
					subtopics: [
						{
							name: "Shell Scripting Tutorial for Beginners",
							link: "https://www.youtube.com/watch?v=cQepf9fY6cE&list=PLS1QulWo1RIYmaxcEqw5JhK3b-6rgdWO_",
						},
						{
							name: "Shell Scripting for DevOps + Hands on projects",
							link: "https://www.youtube.com/watch?v=zsajhz2_50g&list=PLdpzxOOAlwvIZ7u-gtpX_bozrspUbTQ1S",
						},
					],
				},
				{
					name: "Containers",
					subtopics: [
						{
							name: "Containers Explained",
							link: "https://www.youtube.com/watch?v=0qotVMX-J5s",
						},
						{ Dockers: "Docker Tutorial - Kunal Kushwaha(Youtube)" },
					],
				},
				{
					name: "Configuration, Management",
					subtopics: [
						{
							name: "What is Ansible",
							link: "https://www.youtube.com/watch?v=wgQ3rHFTM4E",
						},
						{
							name: "Chef,Puppet,Ansible Course",
							link: "https://www.youtube.com/watch?v=O1s16cYzC10",
						},
					],
				},
				{
					name: "Container, Orchestration",
					subtopics: [
						{
							name: "Complete Kubernetes Course - Hitesh Choudhary(Youtube)",
							link: "https://www.youtube.com/watch?v=7XDeI5fyj3w",
						},
						{
							name: "Learn Kubernetes with hands-on labs - KubeCampus",
							link: "https://kubecampus.io/",
						},
					],
				},
				{
					name: "CI/CD",
					subtopics: [
						{
							name: "What Is CI/CD",
							link: "https://www.youtube.com/watch?v=62N8UiWUdQo&list=PL9gnSGHSqcnoqBXdMwUTRod4Gi3eac2Ak&index=22",
						},
						{ Jenkins: "https://www.youtube.com/watch?v=7KCS70sCoK0" },
					],
				},
				{
					name: "Cloud",
					subtopics: [
						{
							name: "AWS",
							link: "https://docs.aws.amazon.com/",
						},
						{
							name: "Google Cloud",
							link: "https://cloud.google.com/docs",
						},
						{
							name: "Microsoft Azure",
							link: "https://www.youtube.com/watch?v=pTm0iI3_pIQ",
						},
					],
				},
				{
					name: "Projects",
					subtopics: [
						{
							name: "20 Real Time DevOps Projects",
							link: "https://www.youtube.com/playlist?list=PLkWRCY_kK0Gh1NTvgHUE0naF4-mOURp7e",
						},
						{
							name: "11 Projects With Go",
							link: "https://www.youtube.com/watch?v=jFfo23yIWac",
						},
					],
				},
			],
		},
		{
			course: "Frontend Development",
			topics: [
				{
					name: "Introduction",
					subtopics: [
						{
							name: "What is frontend  web development?",
							link: "https://www.youtube.com/watch?v=qyHyFsT7Hig",
						},
					],
				},
				{
					name: "How does Internet work?",
					subtopics: [
						{
							name: "Basics of Internet",
							link: "https://developer.mozilla.org/en-US/docs/Learn/Common_questions/Web_mechanics/How_does_the_Internet_work",
						},
						{
							name: "What is DNS?",
							link: "https://www.cloudflare.com/en-gb/learning/dns/what-is-dns/",
						},
						{
							name: "How the Internet Travel Across Oceans? ",
							link: "https://www.youtube.com/watch?v=yd1JhZzoS6A",
						},
						{
							name: "Internet-CS50's understanding technology",
							link: "https://www.youtube.com/watch?v=n_KghQP86Sw",
						},
						{
							name: "Computer Networking course ",
							link: "https://www.youtube.com/watch?v=IPvYjXCsTg8",
						},
					],
				},
				{
					name: "HTML Tutorial",
					subtopics: [
						{
							name: "Learn HTML",
							link: "https://www.youtube.com/watch?v=kUMe1FH4CHE",
						},
						{ name: "Basics of SEO", link: "https://github.com/seo/guide" },
					],
				},
				{
					name: "Learning CSS",
					subtopics: [
						{
							name: "Introduction to CSS - Web Dev Simplified",
							link: "https://www.youtube.com/playlist?list=PLZlA0Gpn_vH9D0J0Mtp6lIiD_8046k3si",
						},
						{
							name: "A complete guide to Flexbox",
							link: "https://css-tricks.com/snippets/css/a-guide-to-flexbox/",
						},
						{
							name: "A complete guide to CSS Grid",
							link: "https://css-tricks.com/snippets/css/complete-guide-grid/",
						},
						{
							name: "CSS Diner",
							link: "https://flukeout.github.io/",
						},
					],
				},
				{
					name: "Learning Javascript",
					subtopics: [
						{
							name: "Learn JavaScript ",
							link: "https://www.youtube.com/watch?v=PkZNo7MFNFg",
						},
						{
							name: "Learn DOM manipulation ",
							link: "https://www.youtube.com/watch?v=5fb2aPlgoys",
						},
						{
							name: "Learn By reading blogs ",
							link: "https://nikk.hashnode.dev/series/javascript-series",
						},
					],
				},
				{
					name: "React JS",
					subtopics: [
						{
							name: "React Tutorial-Freecodecamp",
							link: "https://www.youtube.com/watch?v=bMknfKXIFA8",
						},
						{
							name: "React Router Tutorial",
							link: "https://www.youtube.com/playlist?list=PLC3y8-rFHvwjkxt8TOteFdT_YmzwpBlrG",
						},
						{
							name: "React Redux Tutorials",
							link: "https://www.youtube.com/playlist?list=PLC3y8-rFHvwheJHvseC3I0HuYI2f46oAK",
						},
					],
				},
				{
					name: "Vue JS",
					subtopics: [
						{
							name: "Tutorial",
							link: "https://www.youtube.com/watch?v=FXpIoQ_rT_c",
						},
						{ name: "Docs", link: "https://vuejs.org/tutorial/#step-1" },
					],
				},
				{
					name: "Angular",
					subtopics: [
						{
							name: "Angular Tutorial ",
							link: "https://www.youtube.com/watch?v=k5E2AVpwsko",
						},
						{
							name: "Docs",
							link: "https://vuejs.org/tutorial/#step-1",
						},
					],
				},

				{
					name: "Projects",
					subtopics: [
						{
							name: "frontendmentor.io",
							link: "https://www.frontendmentor.io/",
						},
						{
							name: "frontendpractice.com",
							link: "https://www.frontendpractice.com/",
						},
					],
				},
			],
		},
		{
			course: "Full Stack Development",
			topics: [
				{
					name: "Getting started with frontend",
					subtopics: [
						{
							name: "Frontend roadmap",
							link: "amader website r frontend link",
						},
					],
				},
				{
					name: "Getting started with backend",
					subtopics: [
						{
							name: "Backend roadmap",
							link: "amader website r backend r link",
						},
					],
				},
				{
					name: "HTML Tutorial",
					subtopics: [
						{
							name: "Learn HTML",
							link: "https://www.youtube.com/watch?v=kUMe1FH4CHE",
						},
						{ name: "Basics of SEO", link: "https://github.com/seo/guide" },
					],
				},
				{
					name: "Learning Typescript",
					subtopics: [
						{
							name: "Learn Typescript ",
							link: "https://www.codecademy.com/learn/learn-typescript",
						},
						{
							name: "Typescript Tutorial ",
							link: "https://www.tutorialspoint.com/typescript/index.htm",
						},
						{
							name: "Docs",
							link: "https://www.typescriptlang.org/docs/",
						},
					],
				},
				{
					name: "Learning GraphQL",
					subtopics: [
						{
							name: "GraphQL tutorial playlist ",
							link: "https://www.youtube.com/playlist?list=PL4cUxeGkcC9iK6Qhn-QLcXCXPQUov1U7f",
						},
					],
				},
				{
					name: "Learning NGiNX",
					subtopics: [
						{
							name: "Use cases of NGiNX",
							link: "https://www.youtube.com/watch?v=WHv_t_yK-QM",
						},
						{
							name: " Tutorial",
							link: "https://www.javatpoint.com/nginx-tutorial",
						},
					],
				},
				{
					name: "Learning Redis",
					subtopics: [
						{
							name: "Redis tutorial",
							link: "https://www.tutorialspoint.com/redis/index.htm#:~:text=Redis%20is%20an%20open%20source,Redis%20is%20written%20in%20C.",
						},
						{ name: "Docs", link: "https://redis.io/docs/about/" },
					],
				},
				{
					name: "CDN",
					subtopics: [
						{
							name: "CDN Tutorial ",
							link: "https://www.youtube.com/watch?v=8zX0rue2Hic",
						},
					],
				},

				{
					name: "Projects",
					subtopics: [
						{
							name: "Build and deploy a modern social media app",
							link: "https://www.youtube.com/watch?v=1RHDhtbqo94",
						},
						{
							name: "Build and deploy a modern netflix clone",
							link: "https://www.youtube.com/playlist?list=PLj-4DlPRT48k8TZ2ZjzbjnAq_wgIwsIab",
						},
					],
				},
			],
		},
		{
			course: "Data-Science",
			topics: [
				{
					name: "What Is Data Science",
					subtopics: [
						{
							name: "what is data science?",
							link: "https://www.ibm.com/topics/data-science",
						},
					],
				},
				{
					name: "Programming Languages",
					subtopics: [
						{
							name: "R",
							link: "https://www.youtube.com/watch?v=_V8eKsto3Ug&feature=youtu.be",
						},
						{
							name: "Python",
							link: "https://www.youtube.com/watch?v=t8pPdKYpowI&feature=youtu.be",
						},
						{
							name: "SQL",
							link: "https://www.youtube.com/watch?v=HXV3zeQKqGY",
						},
					],
				},
				{
					name: "Frameworks",
					subtopics: [
						{
							name: "Pandas",
							link: "https://www.youtube.com/playlist?list=PL-osiE80TeTsWmV9i9c58mdDCSskIFdDS",
						},
						{
							name: "Numpy",
							link: "https://www.youtube.com/watch?v=QUT1VHiLmmI",
						},
						{
							name: "Scipy",
							link: "https://www.youtube.com/watch?v=jmX4FOUEfgU",
						},
					],
				},
				{
					name: "Mathematics for data science",
					subtopics: [
						{
							name: "Mathematics for data science",
							link: "https://www.youtube.com/playlist?list=PLeo1K3hjS3uuKaU2nBDwr6zrSOTzNCs0l",
						},
						{
							name: "Maths for data science -GFG(GeeksForGeeks)",
							link: "https://www.geeksforgeeks.org/maths-for-data-science/",
						},
					],
				},
				{
					name: "Probability:",
					subtopics: [
						{
							name: "Probability and stastics for data science -Khan Academy",
							link: "https://www.khanacademy.org/math/statistics-probability/probability-library",
						},
					],
				},
				{
					name: "Statistics",
					subtopics: [
						{
							name: "Statistics fundamentals -Youtube(StatQuest with Josh Starmer)",
							link: "https://www.youtube.com/playlist?list=PLblh5JKOoLUK0FLuzwntyYI10UQFUhsY9",
						},
						{
							name: "Statistics for data science -Youtube(Great Learning)",
							link: "https://www.youtube.com/watch?v=Vfo5le26IhY",
						},
					],
				},
				{
					name: "Data Manipulation",
					subtopics: [
						{
							name: "Data cleaning",
							link: "https://www.youtube.com/watch?v=bDhvCp3_lYw",
						},
						{
							name: "Data Visualization",
							link: "https://www.youtube.com/watch?v=UO98lJQ3QGI&list=PL-osiE80TeTvipOqomVEeZ1HRrcEvtZB_",
						},
						{
							name: "EDA",
							link: "https://www.youtube.com/watch?v=fHFOANOHwh8",
						},
					],
				},
				{
					name: "Machine Learning",
					subtopics: [
						{
							name: "Machine learning playlist",
							link: "https://www.youtube.com/playlist?list=PLoROMvodv4rMiGQp3WXShtMGgzqpfVfbU",
						},
						{
							name: "ML Documentation",
							link: "https://www.geeksforgeeks.org/machine-learning/",
						},
					],
				},
				{
					name: "Model Deployment",
					subtopics: [
						{
							name: "Flask tutorial -GFG(geeksforgeeks)",
							link: "https://www.geeksforgeeks.org/flask-creating-first-simple-application/",
						},
						{
							name: "Deploy Models with TensorFlow Serving and Flask -Coursera(Amit Yadav)",
							link: "https://www.coursera.org/projects/deploy-models-tensorflow-serving-flask",
						},
					],
				},
				{
					name: "Project Ideas",
					subtopics: [
						{
							name: "Machine Learning Project",
							link: "https://www.youtube.com/watch?v=fiz1ORTBGpY&list=PLfFghEzKVmjvuSA67LszN1dZ-Dd_pkus6",
						},
						{
							name: "End To End ML Project With Dockers,Github Actions And Deployment",
							link: "https://www.youtube.com/watch?v=MJ1vWb1rGwM",
						},
					],
				},
			],
		},
	],
};
export default data;
