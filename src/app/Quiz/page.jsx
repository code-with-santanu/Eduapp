...
export default function Quiz(){
  ...
  return (
    <div className="flex flex-col px-12 py-7 space-y-2 items-center">
      <div className="text-orange-400 shadow-sm bg-white p-3 rounded-lg w-1/2 text-center text-3xl font-extrabold">Quiz on Networking</div>
          ...
          {!showResult ? (
            <div className="">
              ...
              {answer.map((answer, idx) => (
                <li className="text-slate-700 font-medium py-4 px-10 border-b-2 border-slate-100 cursor-pointer active:bg-[#f5e6fc]"
                  key={idx}
              ...
              {checked ? (
                <div onClick={nextQuestion} className="w-28 rounded-md text-center px-4 py-2 bg-slate-100 text-slate-700 hover:bg-slate-200 border border-slate-400 font-medium mt-2 ml-auto cursor-pointer">
            ...
            <div className="">
              ...
              <div className="">
                ...
                <div className="flex px-16 font-bold text-blue-800 py-3 border-b-2 border-slate-200 bg-white">
                  Accuracy: <span className="ml-auto space-x-6">{(result.score / 50) * 100}%</span>
                  </div>
                <div className="flex px-16 font-bold text-orange-400 py-3 border-b-2 border-slate-200 bg-white">
                  Total Questions: <span className="ml-auto space-x-6">{questions.length}</span>
                </div>
                <div className="flex px-16 font-bold text-blue-500 py-3 border-b-2 border-slate-200 bg-white">
                  Total Score: <span className="ml-auto space-x-6">{result.score}</span>
                </div>
                <div className="flex px-16 font-bold text-green-500 py-3 border-b-2 border-slate-200 bg-white">
                  Correct Answer: <span className="ml-auto space-x-6">{result.correctAns}</span>
                </div>
                <div className="flex px-16 font-bold text-red-400 py-3 border-b-2 border-slate-200 bg-white">
                  Wrong Answer: <span className="ml-auto space-x-6">{result.wrongAns}</span>
                </div>
                <div
                  className="flex flex-col items-center"
                  onClick={() => window.location.reload()}
                >
                  <div className=" text-center text-blue-900 font-bold bg-slate-200 w-36 py-2 rounded border border-blue-300 hover:bg-slate-300 mt-4">Retake</div>
                </div>
              </div>
            </div>
          )}
